...
export function drawFogOfWar(ctx:CanvasRenderingContext2D,width:number,height:number,cameraX:number,cameraY:number,discovery:DiscoveryPoint[],playerX:number,playerY:number,inMist:boolean,time:number){
 const canvas=surface(width,height),mask=canvas.getContext('2d')!;mask.clearRect(0,0,canvas.width,canvas.height);mask.globalCompositeOperation='source-over';mask.fillStyle='#07090f';mask.fillRect(0,0,canvas.width,canvas.height);mask.globalCompositeOperation='destination-out';
