type AtlasName='player'|'landmarks'|'terrain'|'expansion'|'special';
...
export function onPixelArtReady(listener:()=>void){listeners.add(listener);if(ready.size===5)queueMicrotask(listener);return()=>{listeners.delete(listener)}}
