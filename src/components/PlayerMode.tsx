...
import {clampMeter,MIST_ENTRY_CONCENTRATION,type DiscoveryPoint,type GameStats} from '../game/gameState';
...
const safeIds=new Set(['house','garden','red-dot']);
...
export default function PlayerMode({player,dialogue,discoveredRegions,onDiscover,onDiscoverProp,game}:PlayerModeProps){
 ...
 useEffect(()=>{let frame=0,last=performance.now();const tick=(now:number)=>{
  ...
  if(!dialogue){const before=player.current;let next=movePlayer(before,keys.current,dt,WORLD.width,WORLD.height);let nextRegion=mapRegions.filter(region=>regionContains(region,next.x,next.y)).sort((a,b)=>regionExtent(a)-regionExtent(b))[0]??null;const wasInMist=isMistCountry(mapRegions.filter(region=>regionContains(region,before.x,before.y)).sort((a,b)=>regionExtent(a)-regionExtent(b))[0]??null,before.x,before.y),enteringMist=isMistCountry(nextRegion,next.x,next.y);
   ...
   else if(rules&&enteringMist&&statsRef.current){const wobble=(statsRef.current.confusion/100)*38*dt;next.x=Math.max(12,Math.min(WORLD.width-12,next.x+(Math.random()-.5)*wobble));next.y=Math.max(12,Math.min(WORLD.height-12,next.y+(Math.random()-.5)*wobble))}
   player.current=next;
  }
  let entered=mapRegions.filter(region=>regionContains(region,player.current.x,player.current.y)).sort((a,b)=>regionExtent(a)-regionExtent(b))[0]??null;let inMist=isMistCountry(entered,player.current.x,player.current.y);
  if(rules&&inMist&&statsRef.current&&statsRef.current.confusion>8&&now>nextWarp.current){nextWarp.current=now+11000+Math.random()*11000;if(Math.random()<.25+statsRef.current.confusion/180){const target=mapRegions.find(region=>region.id===(Math.random()<.52?'booboo':'trauma'))!;player.current={...player.current,x:target.x+(Math.random()-.5)*70,y:target.y+(Math.random()-.5)*70,moving:false};entered=target;inMist=true;setWarning(`THE MISTS FOLD THE ROAD · ${target.name.toUpperCase()}`)}}
  if(rules&&statsRef.current){const stats=statsRef.current;if(inMist){stats.concentration=clampMeter(stats.concentration-dt*1.15);stats.confusion=clampMeter(stats.confusion+dt*(confusionRates[entered?.id??'mist']??1.15))}else stats.confusion=clampMeter(stats.confusion-dt*.28);const points=rules.discovery.current,lastPoint=points[points.length-1],distance=lastPoint?Math.hypot(player.current.x-lastPoint.x,player.current.y-lastPoint.y):999;if(distance>(inMist?68:95))points.push({x:player.current.x,y:player.current.y,radius:inMist?55:112});if(now-lastPublish.current>220){lastPublish.current=now;rules.onStatsChange({...stats})}}
  if(entered?.id!==(activeRegion.current??undefined)){activeRegion.current=entered?.id??null;setCurrentRegion(entered);setNotice(null);if(entered){if(discoveredRegions.current.has(entered.id))setNotice(entered);else{discoveredRegions.current.add(entered.id);if(rules&&inMist&&statsRef.current){statsRef.current.clarity=clampMeter(statsRef.current.clarity+(entered.id==='mist'?3:8));rules.discovery.current.push({x:entered.x,y:entered.y,radius:Math.min(105,regionExtent(entered)+35)});rules.onStatsChange({...statsRef.current})}onDiscover(entered)}}}
  ...
  const camera=playerCamera(player.current.x,player.current.y,width,height,WORLD.width,WORLD.height);ctx.setTransform(d,0,0,d,0,0);ctx.imageSmoothingEnabled=false;ctx.clearRect(0,0,width,height);ctx.save();ctx.translate(-camera.x,-camera.y);const nearby=mapRegions.some(region=>region.id!=='fairy-playground'&&Math.hypot(player.current.x-region.x,player.current.y-region.y)<regionExtent(region)+100);drawMap(ctx,nearby);if(rules)drawFogOfWar(ctx,rules.discovery.current,player.current.x,player.current.y,inMist,now);drawPlayer(ctx,player.current);ctx.restore();if(rules&&inMist)drawMistVeil(ctx,width,height,now,statsRef.current?.confusion??0);drawVignette(ctx,width,height);frame=requestAnimationFrame(tick)
 };frame=requestAnimationFrame(tick);return()=>cancelAnimationFrame(frame)},[dialogue,discoveredRegions,onDiscover,onDiscoverProp,player]);
