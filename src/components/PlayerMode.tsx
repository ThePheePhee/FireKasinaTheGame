...
export interface GameRules {
 ...
 onStatsChange:(stats:GameStats)=>void;onPractice:()=>void;world?:DynamicWorld;onRegionEnter?:(regionId:string)=>void;
}
...
export default function PlayerMode({player,dialogue,discoveredRegions,onDiscover,onDiscoverProp,game}:PlayerModeProps){
 const canvasRef=useRef<HTMLCanvasElement>(null),keys=useRef(new Set<string>()),activeRegion=useRef<string|null>(null),activeProp=useRef<string|null>(null),gameRef=useRef(game),statsRef=useRef(game?.stats),nextWarp=useRef(performance.now()+12000),lastPublish=useRef(0);
 ...
 useEffect(()=>{if(!world)return;const current=world.regions.find(region=>region.id===world.currentRegionId);if(current){player.current={...player.current,x:current.x,y:current.y,moving:false};activeRegion.current=current.id;setCurrentRegion(current);setWarning(`THE WORLD REFORMS · ${world.reason.toUpperCase()}`)}},[player,world?.signature]);
 ...
 useEffect(()=>{let frame=0,last=performance.now();const tick=(now:number)=>{
  ...
  let entered=regions.filter(region=>region.id!=='fairy-playground'&&regionContains(region,player.current.x,player.current.y)).sort((a,b)=>regionExtent(a)-regionExtent(b))[0]??null;let inMist=dynamic?rules?.world?.currentRegionId==='mist'||['trauma','booboo','chasm','counterfeit-crags','credulous-circuit'].includes(entered?.id??''):isMistCountry(entered,player.current.x,player.current.y);
  if(rules&&!dynamic&&inMist&&statsRef.current&&statsRef.current.confusion>8&&now>nextWarp.current){nextWarp.current=now+11000+Math.random()*11000;if(Math.random()<.25+statsRef.current.confusion/180){const target=regions.find(region=>region.id===(Math.random()<.52?'booboo':'trauma'));if(target){player.current={...player.current,x:target.x+(Math.random()-.5)*70,y:target.y+(Math.random()-.5)*70,moving:false};entered=target;inMist=true;setWarning(`THE MISTS FOLD THE ROAD · ${target.name.toUpperCase()}`)}}}
