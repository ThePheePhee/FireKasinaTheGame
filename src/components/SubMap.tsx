...
export default function SubMap({map,onExit}:{map:InteriorMap;onExit:()=>void}){
 ...
 return <section className={`submap submap-${map.theme}`} aria-label={`${map.name} map`}>
  ...
  <div ref={containerRef} className="submap-map"><canvas ref={canvasRef}/><div className="submap-zone-layer">{clickables.map(zone=><button type="button" key={zone.id} aria-label={`Open ${zone.name}`} title={zone.name} className={selected?.id===zone.id?'active':''} style={{left:layout.left+zone.x*layout.scale,top:layout.top+zone.y*layout.scale,width:Math.max(42,zone.w*layout.scale),height:Math.max(42,zone.h*layout.scale)}} onClick={()=>setSelected(zone)}/>)}</div></div>
  {selected&&<aside className="submap-lore" role="dialog" aria-label={selected.name}><button type="button" onClick={()=>setSelected(null)} aria-label="Close lore">×</button><p>MAP LORE</p><h2>{selected.name}</h2><div/><span>◆ {selected.copy}</span>{(selected.references??(selected.reference?[selected.reference]:[])).map(link=><a key={link.label} href={link.url} target="_blank" rel="noreferrer">{link.label} ↗</a>)}</aside>}
