...
export default function SubMap({map,onExit,exitLabel='MAIN MAP',initialFloorIndex=0,position,onReadEntry}:SubMapProps){
 ...
 const captions=useMemo(()=>layoutInteriorCaptions(interiorCaptions(map,floor,floorIndex),viewport,size,[],floor,focusedId??undefined),[map,floor,floorIndex,viewport,size,focusedId]);
 ...
 return <section className={`submap submap-${map.theme}`} aria-label={`${map.name} map`}>
  ...
  <div className="area-view-controls"><span>{wholeFloor?'WHOLE FLOOR':'DETAIL VIEW'}{currentPosition?' · YOU ARE HERE':''}</span><button type="button" onClick={()=>{reset();setFocusedId(null)}} aria-pressed={wholeFloor}>Show whole floor</button><div className="area-zoom-controls" aria-label="Area map zoom controls"><button type="button" aria-label="Zoom in" onClick={()=>zoom(1.5)}>+</button><button type="button" aria-label="Zoom out" onClick={()=>zoom(1/1.5)}>−</button></div></div>
