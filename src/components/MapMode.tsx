...
export default function MapMode({onSelectRegion,onSelectRoute,onClear}:{onSelectRegion:(region:Region)=>void;onSelectRoute:(route:Route)=>void;onClear:()=>void}){
 ...
 const selectRegion=(event:React.MouseEvent,region:Region)=>{event.stopPropagation();if(!suppressClick.current)onSelectRegion(region)};
 const selectRoute=(event:React.MouseEvent,route:Route)=>{event.stopPropagation();if(!suppressClick.current)onSelectRoute(route)};
 const changeZoom=(factor:number)=>setViewport(current=>zoomMap(current,factor,size.width/2,size.height/2,size));

 return <div ref={containerRef} className="map-mode" onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={finishPointer} onPointerCancel={finishPointer} onClick={event=>{if(!suppressClick.current&&(event.target===event.currentTarget||event.target===canvasRef.current))onClear()}}>
