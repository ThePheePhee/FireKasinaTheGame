...
export default function App(){
 const [started,setStarted]=useState(false),[mode,setMode]=useState<Mode>('player'),[selection,setSelection]=useState<Selection>(null),[interior,setInterior]=useState<InteriorMap|null>(null),[overview,setOverview]=useState<SubMapDefinition|null>(null);
 const player=useRef<Player>({...startPlayer});
 useEffect(()=>{const close=(event:KeyboardEvent)=>{if(event.key==='Escape'&&!interior)setSelection(null)};window.addEventListener('keydown',close);return()=>window.removeEventListener('keydown',close)},[interior]);
 const selectRegion=(region:Region|null)=>setSelection(region?{kind:'region',region}:null);
 const region=selection?.kind==='region'?selection.region:null,route=selection?.kind==='route'?selection.route:null,lore=route?pathContent[route.id]:null,interiorEntrance=region?getInteriorMap(region.id):undefined,areaOverview=region?getSubMap(region.id):undefined;
 ...
 return <main className="game">
  ...
  {region&&<section className="bubble dialogue" role="dialog" aria-live="polite" aria-label={region.name}><button aria-label="Close dialogue" onClick={()=>setSelection(null)}>×</button><p>{mode==='player'?'AREA DISCOVERED':'MAP LORE'}</p><h2>{region.name}</h2><div className="rule"/><p className="copy"><span className="dialogue-gem">◆</span>{presentationContent[region.id]}</p>{mode==='player'&&interiorEntrance&&<button className="enter-area" onClick={()=>{setInterior(interiorEntrance);setSelection(null)}}>ENTER THIS AREA →</button>}{mode==='presentation'&&areaOverview&&<button className="enter-area" onClick={()=>{setOverview(areaOverview);setSelection(null)}}>OPEN AREA MAP →</button>}{mode==='presentation'&&!areaOverview&&interiorEntrance&&<button className="enter-area" onClick={()=>{setInterior(interiorEntrance);setSelection(null)}}>EXPLORE AREA →</button>}{presentationReferences[region.id]&&<div className="lore-links"><span>FIELD GUIDE</span>{presentationReferences[region.id]!.map(reference=><a key={reference.label} href={reference.url} target="_blank" rel="noreferrer">{reference.label} ↗</a>)}</div>}<small className="dialogue-hint">{mode==='player'?'CLOSE TO CONTINUE':'TAP OUTSIDE OR × TO CLOSE'}</small></section>}
