...
import {FAIRY_ENTRY_CONCENTRATION,MIST_ENTRY_CONCENTRATION,initialDiscovery,initialGameStats,type GameStats} from './game/gameState';
...
import TraumaLabyrinth from './components/TraumaLabyrinth';
import QuarrelsomeInnMinigame from './components/QuarrelsomeInnMinigame';
import BoohooLagoonMinigame from './components/BoohooLagoonMinigame';
import ChasmDespairMinigame from './components/ChasmDespairMinigame';
...
export default function App(){
 const qaMinigame=import.meta.env.DEV?new URLSearchParams(window.location.search).get('qa'):null;
 const [experience,setExperience]=useState<Experience|null>(qaMinigame?'game':null),[sandboxView,setSandboxView]=useState<SandboxView>('player'),[selection,setSelection]=useState<Selection>(null),[interior,setInterior]=useState<InteriorMap|null>(null),[overview,setOverview]=useState<InteriorMap|null>(null),[gameStats,setGameStats]=useState<GameStats>(initialGameStats),[practicing,setPracticing]=useState(false),[traumaGame,setTraumaGame]=useState(qaMinigame==='trauma'),[innGame,setInnGame]=useState(qaMinigame==='inn'),[lagoonGame,setLagoonGame]=useState(qaMinigame==='lagoon'),[chasmGame,setChasmGame]=useState(qaMinigame==='chasm'),[mistOverride,setMistOverride]=useState(false),[fairyOverride,setFairyOverride]=useState(false);
 ...
 const toggleMistExploration=useCallback((checked:boolean)=>{setMistOverride(checked);if(checked)setGameStats(stats=>({...stats,concentration:Math.max(stats.concentration,MIST_ENTRY_CONCENTRATION+1)}))},[]);
 const toggleFairyExploration=useCallback((checked:boolean)=>{setFairyOverride(checked);if(checked)setGameStats(stats=>({...stats,concentration:Math.max(stats.concentration,FAIRY_ENTRY_CONCENTRATION+1),clarity:Math.max(stats.clarity,81)}))},[]);
 ...
 return <main className={`game experience-${experience}`}>
  ...
  {isGame&&<GameHud stats={gameStats} mistOverride={mistOverride} fairyOverride={fairyOverride} onMistOverride={toggleMistExploration} onFairyOverride={toggleFairyExploration}/>}<aside className="help">{view==='player'?<>MOVE <kbd>WASD</kbd> <kbd>↑↓←→</kbd></>:<>TAP REGIONS & PATHS · DRAG · PINCH</>}</aside>
  {region&&<section className="bubble dialogue" role="dialog" aria-live="polite" aria-label={region.name}><button aria-label="Close dialogue" onClick={()=>setSelection(null)}>×</button><p>{view==='player'?'AREA DISCOVERED':'MAP LORE'}</p><h2>{region.name}</h2><div className="rule"/><p className="copy"><span className="dialogue-gem">◆</span>{presentationContent[region.id]}</p>{isGame&&region.id==='red-dot'&&<button className="enter-area practice-red-dot" onClick={()=>{setPracticing(true);setSelection(null)}}>PRACTISE WITH THE RED DOT →</button>}{isGame&&region.id==='trauma'&&<button className="enter-area" onClick={()=>{setTraumaGame(true);setSelection(null)}}>ENTER THE LABYRINTH →</button>}{view==='player'&&interiorEntrance&&<button className="enter-area" onClick={()=>{setInterior(interiorEntrance);setSelection(null)}}>ENTER THIS AREA →</button>}{view==='presentation'&&interiorEntrance&&<button className="enter-area" onClick={()=>{setOverview(interiorEntrance);setSelection(null)}}>OPEN AREA MAP →</button>}{view==='presentation'&&interiorEntrance&&<button className="enter-area secondary" onClick={()=>{setInterior(interiorEntrance);setSelection(null)}}>PLAY THIS AREA →</button>}{presentationReferences[region.id]&&<div className="lore-links"><span>FIELD GUIDE</span>{presentationReferences[region.id]!.map(reference=><a key={reference.label} href={reference.url} target="_blank" rel="noreferrer">{reference.label} ↗</a>)}</div>}<small className="dialogue-hint">{view==='player'?'CLOSE TO CONTINUE':'TAP OUTSIDE OR × TO CLOSE'}</small></section>}
  ...
  {isGame&&region?.id==='inn'&&<button className="inn-entry-float" onClick={()=>{setInnGame(true);setSelection(null)}}>JOIN THE ARGUMENT →</button>}
  {isGame&&region?.id==='booboo'&&<button className="inn-entry-float" onClick={()=>{setLagoonGame(true);setSelection(null)}}>FLY BETWEEN THE SADNESSES →</button>}
  {isGame&&region?.id==='chasm'&&<button className="inn-entry-float" onClick={()=>{setChasmGame(true);setSelection(null)}}>ATTEMPT THE CROSSING →</button>}
  {interior&&<InteriorMode map={interior} onExit={()=>setInterior(null)}/>} {overview&&<SubMap map={overview} onExit={()=>setOverview(null)}/>} {practicing&&<RedDotMinigame stats={gameStats} onChange={onGameStatsChange} onExit={()=>setPracticing(false)}/>} {traumaGame&&<TraumaLabyrinth stats={gameStats} onChange={onGameStatsChange} onExit={()=>setTraumaGame(false)}/>} {innGame&&<QuarrelsomeInnMinigame stats={gameStats} onChange={onGameStatsChange} onExit={()=>setInnGame(false)}/>} {lagoonGame&&<BoohooLagoonMinigame stats={gameStats} onChange={onGameStatsChange} onExit={()=>setLagoonGame(false)}/>} {chasmGame&&<ChasmDespairMinigame stats={gameStats} onChange={onGameStatsChange} onExit={()=>setChasmGame(false)}/>}
 </main>;
}
