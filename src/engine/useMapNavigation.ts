...
import {fitMap,focusMapTarget,resizeMap,zoomMap,type MapBounds,type MapFocusTarget,type ScreenSize} from './mapViewport';

export function useMapNavigation(container:RefObject<HTMLDivElement|null>,bounds:MapBounds,onActivate:(target:string)=>void,enabled=true){
 const [size,setSize]=useState<ScreenSize>({width:1,height:1}),[viewport,setViewport]=useState<View>({x:0,y:0,scale:1});
 const current=useRef({size,viewport,bounds,onActivate,enabled}),gesture=useRef(new MapGesture());
 current.current={size,viewport,bounds,onActivate,enabled};
 ...
 useEffect(()=>{
  ...
  const wheel=(event:WheelEvent)=>{if(!current.current.enabled||(event.target as HTMLElement).closest('.map-tools'))return;event.preventDefault();const rect=element.getBoundingClientRect(),state=current.current,delta=event.deltaY*(event.deltaMode===1?16:event.deltaMode===2?state.size.height:1);update(zoomMap(state.viewport,Math.exp(-Math.max(-150,Math.min(150,delta))*.002),event.clientX-rect.left,event.clientY-rect.top,state.size,state.bounds))};
  const cancel=()=>gesture.current.cancel();
  element.addEventListener('wheel',wheel,{passive:false});window.addEventListener('blur',cancel);document.addEventListener('visibilitychange',cancel);
  return()=>{observer.disconnect();element.removeEventListener('wheel',wheel);window.removeEventListener('blur',cancel);document.removeEventListener('visibilitychange',cancel)};
 },[container,bounds.width,bounds.height]);
 useEffect(()=>{if(!enabled)gesture.current.cancel()},[enabled]);
 ...
 const finish=(event:ReactPointerEvent<HTMLDivElement>,cancel=false)=>{
  ...
  if(target&&current.current.enabled)current.current.onActivate(target);
 };
 return{size,viewport,focus:(target:MapFocusTarget)=>{gesture.current.cancel();update(focusMapTarget(target,current.current.size,current.current.bounds))},reset:()=>{gesture.current.cancel();update(fitMap(current.current.size,current.current.bounds))},zoom:(factor:number)=>{const state=current.current;update(zoomMap(state.viewport,factor,state.size.width/2,state.size.height/2,state.size,state.bounds))},bindings:{
  onPointerDown:(event:ReactPointerEvent<HTMLDivElement>)=>{if(!current.current.enabled||event.button!==0||(event.target as HTMLElement).closest('.map-tools'))return;const target=(event.target as HTMLElement).closest<HTMLElement>('[data-map-target]')?.dataset.mapTarget??'clear';gesture.current.down(event.pointerId,local(event),current.current.viewport,target);event.currentTarget.setPointerCapture(event.pointerId)},
